...
function generateAccessToken(id, name, islogin){
    return jwt.sign({userId:id, name: name, islogin: islogin}, 'secretkey')
}
      ...
      if (user) {
        bcrypt.compare(entered_password,user.password,(err,result)=>{
            ...
            if (result===true){
                ...
                return res.status(200).json({message:'User login sucessful', token: generateAccessToken(user.dataValues.id,user.dataValues.name,true)});  
            }
            else{
                console.log(result)
...
const logout = async (req,res)=>{
    try{
        console.log("yes 99999")
        const token = req.headers.authorization;
        decoded = parseJwt (token)
        console.log(decoded)
        const islogin  = decoded.islogin
        const id = decoded.id
        const name = decoded.name
        return res.status(200).json({message:'User loggedout successfully',token : generateAccessToken(id,name,false)})
